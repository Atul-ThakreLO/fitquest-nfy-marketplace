...
async function graphQuery<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
  ...
  if (json.errors?.length) {
    throw new Error(`Graph query errors: ${json.errors.map(e => e.message).join(', ')}`)
  }
...
class GraphService {
   ...
   */
  async getActiveListings(skip: number, take: number, rarity?: number, sort?: string): Promise<Listing[]> {
  ...
  async getActiveAuctions(): Promise<Auction[]> {
    ...
    return data.auctions.map(a => ({
      ...
      highestBidder: (a.highestBidder?.id ?? '0x0000000000000000000000000000000000000000') as `0x${string}`,
