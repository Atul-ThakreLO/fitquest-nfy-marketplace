...
export const auctionRoutes = (contractService: ContractService) =>
  new Elysia({ prefix: "/auction", tags: ["auction"] })
    ...
    .get(
      ...
      async ({ params, status }) => {
        try {
          ...
          const history = await graphService.getTokenAuctionHistory(tokenId);
          ...
          return status(500, {
            success: false,
            message: "Failed to fetch auction token history",
          });
        }
      },
      {
        params: t.Object({ tokenId: t.String() }),
        detail: { summary: "Get all auction bid history points for a token" },
      },
    )
    ...
    .post(
      ...
      },
      {
        ...
        detail: {
          summary:
            "Trigger auction finalization (callable by anyone after deadline)",
        },
      },
    );
