...
import { resolveIPFS, truncateAddress, formatEth, resolveImageIPFS } from '@/lib/utils'
...
export function NFTDetail({ tokenId }: NFTDetailProps) {
  ...
  const { data: token, isLoading: tokenLoading, isError: tokenError } = useNFT(tokenId)
  const { data: listing } = useListing(tokenId)
  const { data: history } = useListingHistory(tokenId);
  ...
  if (tokenError || !token) return <ErrorState message="NFT not found or failed to load." />
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
      ...
      <div className="flex flex-col gap-6">
        ...
        <div>
          ...
          <h1 className="text-3xl font-bold text-white">Token #{token.tokenId.toString()}</h1>
          <p className="text-zinc-400 mt-1">
            Owned by{' '}
            <Link href={`/profile/${token.owner}`} className="text-orange-400 hover:underline">
          ...
          {isListed && listing ? (
            <>
              <div>
                ...
                <p className="text-3xl font-bold text-orange-400">{formatEth(listing.price)}</p>
              </div>
              {!isOwner && <BuyButton tokenId={token.tokenId} price={listing.price} />}
              {isOwner && <CancelListingButton tokenId={token.tokenId} />}
            </>
          ) : isOwner ? (
            <ListingForm tokenId={token.tokenId} />
          ) : (
            <p className="text-zinc-400 italic">Not currently listed for sale.</p>
          )}
        </div>

        {/* Attributes */}
        <div>
          <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-3">Attributes</h3>
          <div className="grid grid-cols-3 gap-2">
            {[
              { trait: 'Rarity Tier', value: token.rarity.toString() },
              { trait: 'First Listed', value: token.firstListed ? 'Yes' : 'Pending' },
            ].map(attr => (
              <div key={attr.trait} className="glass rounded-xl p-3 text-center">
        ...
        {history && history.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-3">Price History</h3>
